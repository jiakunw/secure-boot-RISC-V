...
vt_log_dir() {
    local repo="$1"
    # This bundle lives at $repo/tests/validation_tests/, not at $repo/.
    # Logs go under that bundle, not at the repo root, to keep all
    # validation-test artifacts in one place.
    mkdir -p "$repo/tests/validation_tests/logs"
    printf '%s\n' "$repo/tests/validation_tests/logs"
}
...
vt_run_sim() {
    ...
    # Do NOT use `stdbuf -oL`: Verilator's generated C++ uses iostreams,
    # and libstdbuf's LD_PRELOAD'd stdio override silently swallows the
    # sim's output when stdout is a file. We hit this bug across multiple
    # test scripts — drop stdbuf and let sim's $finish flush naturally.
    set +e
    timeout "$timeout_s" "$sim" "+payload=$recovery" "$kernel" > "$log" 2>&1
    local rc=$?
    set -e
