...
make_base_copy() {
    rm -rf "$BASE"
    cp -a "$REPO" "$BASE"
    rm -rf "$BASE/.git" "$BASE/tests/validation_tests/logs"
...
run_expect_fail() {
    ...
    echo "-- $name --"
    (cd "$CASE" && "$@")
    if python3 "$CASE/tests/validation_tests/tools/image_tool.py" --repo "$CASE" check-artifacts >/tmp/vt_static_check.out 2>&1; then
...

{
    ...
    run_expect_fail "manifest magic byte changed" \
        python3 tests/validation_tests/tools/image_tool.py --repo . patch-flash-byte --offset 0x0 --xor 0xff

    run_expect_fail "public key byte changed in flash" \
        python3 tests/validation_tests/tools/image_tool.py --repo . patch-flash-byte --offset 0xa5 --xor 0xff

    run_expect_fail "kernel byte changed in flash" \
        python3 tests/validation_tests/tools/image_tool.py --repo . patch-flash-byte --offset 0xc0 --xor 0xff

    run_expect_fail "manifest payload_size changed without rebuilding flash/signature" \
        python3 tests/validation_tests/tools/image_tool.py --repo . patch-manifest-field --field payload_size --value 0
} 2>&1 | tee "$LOG"
